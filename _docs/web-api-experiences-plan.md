...
| Status | API | Treatment |
|---|---|---|
| 🟢 used | HTML-in-Canvas API (experimental) | Attributed article quote-card PNG export; native rendering when available, SVG-to-PNG fallback without browser flags, and semantic HTML preview/copying. Discovery is awarded only after successful native export. See [use-case decision, implementation, and availability notes](html-in-canvas-quote-cards.md). |
