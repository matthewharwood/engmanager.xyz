...
The `portrait-pdf-v2` brief requests a concise personal portrait, an editable first-person passage, and one actual downloadable PDF. The portrait leads with the person: no completion recap, inline evidence IDs, instrument names, scoring tutorial, repeated caveats, exercises, or full questionnaire appendix. Exact numeric results, compact evidence notes, limitations, and required source attribution belong in the back matter. All selected raw answers remain in the input kit for grounding.
...
Every published `personality/v1/`, `personality/ai/v1/`, and `personality/v2/` file stays byte-identical. The revised presentation lives in `personality/v3/` and imports the frozen scientific scoring, storage, codecs, definitions, and PDF modules. Legacy shared links continue through the original app. The new UI's ordinary saved drafts use the same database and exact snapshot protocol.

The v3 service worker installs the current presentation plus the public assets required for legacy reports. It accepts the recognized v1, v2, and v3 installer identities, verifies the selected manifest, and preserves earlier caches. No answer-bearing URL is used as a cache key. Tests pin the original asset digests so a presentation revision cannot silently invalidate published links.
...
The editorial acceptance method is documented in [portrait-quality-evaluation.md](portrait-quality-evaluation.md). It uses the Marcus example for style only, with neutral, divergent, incomplete, and hostile-context synthetic cases. Passing prompt-contract tests is not evidence that prose quality has improved; actual generated outputs must be reviewed against the rubric.
