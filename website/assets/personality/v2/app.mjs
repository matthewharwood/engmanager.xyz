...
function report(){
  ...
  const pdfButton=button('Download PDF',async()=>{pdfButton.disabled=true;pdfButton.textContent='Preparing your PDF…';try{const {downloadPdf}=await import('../v1/pdf.mjs');await flush();await downloadPdf(reportModel(),{size:size.value});}finally{pdfButton.disabled=false;pdfButton.textContent='Download PDF';}});
  ...
  const files=el('details',null,{class:'report-optional-files'});files.append(el('summary','Backups and other exports'),p('The report kit is a writing prompt with your evidence. JSON backups are for restoring this assessment in the app. A PDF contains your scored report and any kept local reflection.'));
