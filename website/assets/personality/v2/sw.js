import {RELEASE} from '/assets/personality/v2/release.mjs';
import {AI_CACHE} from '/assets/personality/v1/ai-cache-config.mjs';
import {releaseSource} from '/assets/personality/v2/release-format.mjs';

const ASSET_ROOT = '/assets/personality/v2/';
...
const READY_PATH = '/personality/.offline-ready-v2';
...
const cacheNamePromise = digestPromise.then(digest => `personality-v2-${digest}`);
...
async function installRelease(progress) {
  ...
  try {
    ...
    await target.put(urlFor(READY_PATH), new Response(JSON.stringify({v: 2, releaseDigest: digest, assets: publicPaths.size, routes: ROUTES.length}), {
  ...
  event.waitUntil((async () => {
    ...
    try {
      ...
      if (source.origin !== origin || !INSTALL_SOURCES.has(source.pathname) ||
          Object.keys(event.data).length !== 2 || ![digest, RELEASE.legacy.releaseDigest].includes(event.data.releaseDigest)) {
        throw new Error('Reload this assessment before installing its offline release.');
      }
      // A frozen v1 client can install this compatible superset. Echo its exact
      // requested digest while retaining both complete public releases.
