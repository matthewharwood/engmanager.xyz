...
export function mountReportKit(root,{state,enhancement=null,recordId=null,canSave=false,flush=async()=>{},pendingWrites=Promise.resolve(),onPending=()=>{}}) {
  ...
  panel.append(el('p','01 / FROM RESULTS TO A PERSONAL NARRATIVE',{class:'eyebrow'}),el('h2','Your whole profile. One file.',{id:'report-kit-title'}),el('p','Turn these results into a candid bird’s-eye view: the patterns behind the averages, the conditions you may prefer, and words to help someone understand you.'));
  const steps=el('ol',null,{class:'kit-steps'});
  for(const [title,body] of [
    ['Download your kit','One Markdown file includes the writing brief, every selected answer, all available scores and facets, and the source notes.'],
    ['Drop it into your LLM','Start a fresh chat in an LLM you choose. Attach the file and ask it to follow the enclosed brief. If attachments are unavailable, copy and paste the complete kit.'],
    ['Make the story yours','The brief asks for a full narrative, a candid first-person script, and a standalone HTML report. Check the interpretation, revise anything that does not fit, then save the HTML.']
  ...
  const context=el('textarea',null,{id:'kit-context',rows:'3',maxlength:'512',placeholder:'For example: I want a collaborator to understand how I work, including what is easy to miss.'});
  const fields=el('div',null,{class:'kit-fields'});
  fields.append(field('Name in the generated report',name,'Leave blank to use “this person.”'),field('What would you like someone to understand?',context,'Only write details you want included in the downloaded file. These notes do not change your scores.'));
  ...
  downloadButton.addEventListener('click',()=>action(kit=>{download(kit);status.textContent='Report kit ready in your downloads. Attach it to a fresh chat and ask: “Follow the single-person brief in this file. Create the report and standalone HTML.”';}));
  ...
  copyButton.addEventListener('click',()=>action(async kit=>{
    try { await navigator.clipboard.writeText(kit.text);status.textContent='Complete prompt and evidence copied. Paste into a fresh chat in your chosen LLM.'; }
  ...
  later.append(el('summary','Later: explore two profiles together'),el('p','The main kit is for one person. For a later collaboration conversation, use two separately reviewed kits shared with both people’s agreement. This optional prompt keeps their evidence separate and asks for a combined HTML discussion guide.'));
