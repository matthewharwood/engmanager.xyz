import {RELEASE} from '/assets/personality/v1/release.mjs';
import {AI_CACHE} from '/assets/personality/v1/ai-cache-config.mjs';

const ASSET_ROOT = '/assets/personality/v1/';
...
const READY_PATH = '/personality/.offline-ready-v1';
const MAX_FILE_BYTES = 8 * 1024 * 1024;
const origin = self.location.origin;
const urlFor = path => new URL(path, origin).href;
const publicPaths = new Set([...Object.keys(RELEASE.assets).map(path => ASSET_ROOT + path), RELEASE_PATH]);
...
const cacheNamePromise = digestPromise.then(digest => `personality-v1-${digest}`);
...
function expectedReleaseSource() {
...
async function installRelease(progress) {
  ...
  const paths = [...Object.keys(RELEASE.assets).map(path => ASSET_ROOT + path), RELEASE_PATH, ...ROUTES];
  ...
  try {
    const jobs = paths.map(path => async () => {
      const relative = path.startsWith(ASSET_ROOT) ? path.slice(ASSET_ROOT.length) : null;
      const response = await fetchPublic(path, relative ? RELEASE.assets[relative] : undefined);
    ...
    await target.put(urlFor(READY_PATH), new Response(JSON.stringify({v: 1, releaseDigest: digest, assets: publicPaths.size, routes: ROUTES.length}), {
  ...
  event.waitUntil((async () => {
    ...
    try {
      ...
      if (source.origin !== origin || !INSTALL_SOURCES.has(source.pathname) ||
          Object.keys(event.data).length !== 2 || event.data.releaseDigest !== digest) {
        throw new Error('Reload this assessment before installing its offline release.');
      }
      const progress = (completed, total) => port.postMessage({type: 'OFFLINE_PROGRESS', releaseDigest: digest, completed, total});
      if (!installation) installation = installRelease(progress).finally(() => { installation = null; });
      const result = await installation;
      await self.skipWaiting();
      port.postMessage({type: 'OFFLINE_READY', releaseDigest: digest, ...result});
    } catch (error) {
...
async function navigation(request, path) {
  try {
    ...
    return await (await installedCache())?.match(urlFor(path)) || response;
  } catch {
    return await (await installedCache())?.match(urlFor(path)) || new Response('This assessment is not available offline yet. Reconnect and choose “Make available offline” from your library.', {
...
self.addEventListener('fetch', event => {
  ...
  if (request.mode === 'navigate' && ROUTES.includes(url.pathname)) {
    ...
    event.respondWith((async () => await (await installedCache())?.match(urlFor(url.pathname)) || fetch(request))());
