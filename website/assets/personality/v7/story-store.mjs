...
import {validateStory as validateLegacyStory} from '../v6/story-store.mjs';

// Keep the published v6 record readable by older tabs and offline releases.
// Its answer to the revised question cannot silently become a new response.
const key = id => `story-atlas-v2:${id}`;
const legacyKey = id => `story-atlas:${id}`;
const LEGACY_REVISED_ITEM = 'type-candidate-ei-12';
...
export async function loadStory(id, sources, {databaseName=DATABASE_NAME}={}) {
  ...
  try {
    ...
    const legacy=saved?null:await tx.objectStore('notes').get(legacyKey(id));
    await tx.done;
    if (!draft) throw new Error('This assessment no longer exists.');
    if (saved) {
      if (saved.kind!=='story-atlas-v2'||saved.draftId!==id||!Number.isSafeInteger(saved.revision)||saved.revision<1) throw new Error('Saved story is invalid.');
      return {revision:saved.revision,value:validateStory(saved.value,sources),migrated:false,revisedQuestion:false};
    }
    if (!legacy) return {revision:0,value:emptyStory(),migrated:false,revisedQuestion:false};
    if (legacy.kind!=='story-atlas-v1'||legacy.draftId!==id||!Number.isSafeInteger(legacy.revision)||legacy.revision<1) throw new Error('Saved story is invalid.');
    const value=structuredClone(validateLegacyStory(legacy.value,sources));
    const revisedQuestion=Object.hasOwn(value.type,LEGACY_REVISED_ITEM);
    delete value.type[LEGACY_REVISED_ITEM];
    // The UI persists this initial migrated value through the normal guarded
    // save queue. Reading alone never writes or deletes an existing record.
    return {revision:0,value:validateStory(value,sources),migrated:true,revisedQuestion};
  } finally {db.close();}
}
...
export async function saveStory(id, expectedRevision, value, sources, {databaseName=DATABASE_NAME}={}) {
  ...
  try {
    ...
    try {
      ...
      if (prior && (prior.kind!=='story-atlas-v2'||prior.draftId!==id||!Number.isSafeInteger(prior.revision)||prior.revision<1)) throw new Error('Saved story is invalid.');
      ...
      await tx.objectStore('notes').put({kind:'story-atlas-v2',draftId:id,revision:revision+1,value:clean},key(id));
      await tx.done;
      return {revision:revision+1,value:clean,migrated:false,revisedQuestion:false};
...
export async function loadStorySources() {
  ...
  const data=await Promise.all(names.map(async name=>{
    const response=await fetch(`/assets/personality/v7/data/${name}.json`,{credentials:'omit'});
