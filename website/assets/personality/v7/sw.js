import {RELEASE} from '/assets/personality/v7/release.mjs';
import {AI_CACHE} from '/assets/personality/v1/ai-cache-config.mjs';
import {releaseSource} from '/assets/personality/v7/release-format.mjs';

const ASSET_ROOT = '/assets/personality/v7/';
...
const READY_PATH = '/personality/.offline-ready-v7';
...
const cacheNamePromise = digestPromise.then(digest => `personality-v7-${digest}`);
...
async function installRelease(progress) {
  ...
  try {
    ...
    await target.put(urlFor(READY_PATH), new Response(JSON.stringify({v: 7, releaseDigest: digest, assets: publicPaths.size, routes: ROUTES.length}), {
...
self.addEventListener('message', event => {
  ...
  event.waitUntil((async () => {
    ...
    try {
      ...
      // Frozen v1–v6 clients can install this compatible superset. Echo
