import {BANK, MODULES} from '../v1/bank.mjs';
import {createState, validateState, selectedSlots, itemText, score} from '../v1/core.mjs';
import {openStore, serializeBackup} from '../v1/store.mjs';
...
import {mountFormNavigation} from './form-navigation.mjs';
import {EXPERIMENTS} from '../v1/report-content.mjs';
import {createContext,assessmentBasis} from '../v1/enhancement.mjs';
import {mountReflection} from '../v1/reflection-ui.mjs';
import {encodeEnhancedSnapshot,readEnhancedIngress,exportReflection,importReflection} from '../v1/enhancement-share.mjs';
...
let reflectionContext=createContext(),enhancement=null,disposeReflection=null,disposeReportKit=null,disposeStoryStudio=null,disposePrefaceName=null,disposeFormNavigation=null;
...
async function flush() { await queue; await pendingKit; }
...
function prepare() {
  main.append(heading('01 / Before you begin','Start with your story.','Add a few details that make the report yours, then continue to one complete questionnaire. You can leave a background question blank and come back later.'));
  const map=el('nav',null,{class:'preface-map','aria-label':'Questionnaire sections'});
  for(const [label,href] of [['Your birthday','#story-symbols-editor'],['Your background','#story-background'],['Your natural preferences','#story-type-editor'],['The main questionnaire','#scored-profiles']]){
  ...
  form.append(el('h2','Your complete questionnaire'),p('170 scored items across Big Five traits, work interests, and personal values. Your answers save as you go. The story and 48 preference questions add context without changing scientific scores. The values portraits use neutral pronouns unless your own story supplies another preference.'));
  if(!readOnly&&state.modules.length!==MODULES.length){state.modules=MODULES.map(module=>module.id);persist();}
  form.append(actions(button(state.cursor?'Resume questionnaire →':'Begin questionnaire →',()=>navigate('test'))));
...
function questionnaire() {
  ...
  for(const item of items){
    const fieldset=el('fieldset',null,{class:'question-card',id:`question-${item.slot}`,disabled:readOnly,'data-question-id':item.id,'data-question-answered':String(state.responses[item.slot]!==null)});
    ...
    for(const option of module.options){
      ...
      input.addEventListener('change',()=>{state.responses[item.slot]=option.value;state.skipped[item.slot]=false;state.cursor={module:item.module,slot:item.slot};state.view='assessment';state.reportDate=null;indicator.textContent='Answered';indicator.dataset.answered='true';fieldset.dataset.questionAnswered='true';persist();updateProgress();});
      options.append(el('label',null,{class:'answer-option'},[input,el('span',option.label)]));
    }
    fieldset.append(options,el('div',null,{class:'question-tools'},[indicator]));main.append(fieldset);
  }
  const slots=selectedSlots(state), first=slots.indexOf(items[0].slot), after=slots.indexOf(items.at(-1).slot)+1;
  main.append(el('div',null,{class:'actions quiz-actions'},[p(readOnly?'Read-only snapshot. Save a copy to change answers.':'No time limit. Leave an item blank and return later; incomplete scales stay unscored.','small'),button('← Back',async()=>{if(first===0)return navigate('prepare');await jump(slots[Math.max(0,first-5)]);},true),button(after===slots.length?'Review answers →':'Next five →',async()=>{if(after===slots.length)return navigate('review');if(BANK.items[slots[after]].module!==module.id)play('section-complete');await jump(slots[after]);})]));
}
function answerStatus(slot){return state.responses[slot]!==null?'Answered':state.skipped[slot]?'Skipped':'Not answered';}
function updateProgress(){const coverage=score(state).completion;const progress=main.querySelector('.assessment-progress');if(progress)progress.value=coverage.answered;const text=main.querySelector('.progress-meta span');if(text)text.textContent=`${coverage.answered} of ${coverage.total} answered`;}
...
function review(){
  ...
  filter.addEventListener('change',rows);main.append(el('label',null,{class:'check-line',for:'unanswered-only'},[filter,document.createTextNode('Show unanswered items only')]),list);rows();
}

function report(){
  const header=heading('04 / YOUR REPORT','Your personality atlas.','One report that brings measured patterns, creative views, and your chosen story details together. Open a chapter to go deeper, or print a reading copy.');
  header.classList.add('report-hero');
  main.append(header);
  const model=reportModel();
  const size=el('select',null,{'aria-label':'PDF paper size'},[el('option','US Letter',{value:'letter'}),el('option','A4',{value:'a4'})]);
  const pdfButton=button('Download scores PDF',async()=>{pdfButton.disabled=true;pdfButton.textContent='Preparing your PDF…';try{const {downloadPdf}=await import('../v1/pdf.mjs');await flush();await downloadPdf(reportModel(),{size:size.value});}finally{pdfButton.disabled=false;pdfButton.textContent='Download scores PDF';}});
  main.append(el('div',null,{class:'report-toolbar'},[p('Your scored report','small'),pdfButton,size,button('Share this report',()=>navigate('share'),true)]));
  const content=el('div');main.append(content);renderReport(content,model);
  let studio=null;
  if(!readOnly){
    studio=mountStoryStudio(main,{recordId:record?.id??null,canSave:Boolean(db&&!saveFailure),onPending:trackPending,
      onChange:()=>renderReport(content,reportModel(),studio?.snapshot()??null)});
    disposeStoryStudio=studio.dispose;
  }
  disposeReportKit=mountReportKit(main,{state:clone(state),enhancement:currentEnhancement(),recordId:record?.id??null,
    canSave:Boolean(db&&!saveFailure&&!readOnly),flush,pendingWrites:pendingKit,onPending:trackPending,story:()=>studio?.snapshot()??null,storyReady:studio?.ready??Promise.resolve()});
  if(!readOnly){
    const local=el('details',null,{id:'local-reflection-tools',class:'report-optional-tools'});
    local.append(el('summary','Optional: reflect and experiment entirely on this device'),p('The report kit above works with the LLM you choose. These separate tools keep their processing here, with reviewed suggestions and optional local model setup.'));
    const experiments=el('details',null,{class:'report-optional-experiments'});
    experiments.append(el('summary','Choose an experiment to keep in your scored report'));
    const panel=el('section',null,{class:'panel',id:'choose-experiments'},[el('h2','A small next step'),p('Your selections are saved in the report and its full snapshot. They do not affect your scores.')]);
    for(const experiment of EXPERIMENTS){const input=el('input',null,{type:'checkbox',checked:state.blocks.includes(experiment.id)});input.addEventListener('change',()=>{state.blocks=EXPERIMENTS.filter(e=>e.id===experiment.id?input.checked:state.blocks.includes(e.id)).map(e=>e.id).sort();persist();renderReport(content,reportModel(),studio?.snapshot()??null);});panel.append(el('label',null,{class:'check-line'},[input,document.createTextNode(experiment.title)]));}
    experiments.append(panel);local.append(experiments);main.append(local);
    const basis=assessmentBasis(state),owner=record?.id;
    disposeReflection=mountReflection(local,{state:clone(state),context:reflectionContext,enhancement:currentEnhancement(),onSaved:render,save:(context,value)=>{
      pending++;updateStatus();
  ...
  const files=el('details',null,{class:'report-optional-files'});files.append(el('summary','Backups and other exports'),p('The report kit is a writing prompt with your evidence and the story details you entered. JSON backups restore the scored assessment in this app. The scores PDF contains your numeric report and any kept local reflection. Your chosen LLM creates the illustrated atlas PDF from the kit.'));
  files.append(actions(button('Answer backup',backup,true),button('Print',()=>window.print(),true)));
  if(currentEnhancement())files.append(actions(button('Download full reflection JSON',()=>download(exportReflection(state,currentEnhancement()),'big-six-seven-reflection.json','application/json'),true)),p('The reflection file includes selected answers, kept reflection text, and its cited excerpts. Unused private notes are excluded.','small'));
  main.append(files);
}
...
function render(){
  disposeFormNavigation?.();disposeFormNavigation=null;
  ...
  if(!readOnly&&['prepare','test'].includes(next))disposeFormNavigation=mountFormNavigation(main,{optional:next==='prepare',remaining:()=>next==='test'?selectedSlots(state).filter(slot=>state.responses[slot]===null).length:0,jumpRemaining:()=>run(async()=>{const slots=selectedSlots(state),cursor=state.cursor?.slot??-1;const slot=slots.find(slot=>slot>cursor&&state.responses[slot]===null)??slots.find(slot=>state.responses[slot]===null);if(slot!==undefined)await jump(slot);})});
}
...
function shell(){
  ...
  document.addEventListener('click',event=>{const a=event.target.closest('a');if(!a||event.defaultPrevented||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey||event.button!==0||a.target||a.download||a.hasAttribute('data-document-navigation'))return;if(a.hasAttribute('data-report-anchor')){event.preventDefault();const target=main.querySelector(a.getAttribute('href'));target?.scrollIntoView({block:'start'});if(target instanceof HTMLElement){target.setAttribute('tabindex','-1');target.focus({preventScroll:true});}return;}if(a.classList.contains('skip-link')){event.preventDefault();main.focus();main.scrollIntoView({block:'start'});return;}const url=new URL(a.href,location.href);if(url.origin!==location.origin)return;const next=url.pathname.split('/')[2];if(url.pathname.startsWith('/personality/')&&labels[next]&&!url.hash&&!url.search){event.preventDefault();if(bootReady)run(()=>navigate(next));}});
