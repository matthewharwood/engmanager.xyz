...
export function mountPrefaceName(root,{recordId=null,canSave=false,onPending=()=>{}}={}) {
  const panel=el('section',null,{class:'panel preface-name',id:'story-question-name','aria-labelledby':'preface-name-title','data-question-id':'report-name','data-question-answered':'false',tabindex:'-1'});
  ...
  const updateAnswer=()=>{panel.dataset.questionAnswered=String(Boolean(name.value.trim()));};
  ...
  if(canSave&&recordId){
    loadKitSettings(recordId).then(saved=>{
      ...
      if(!disposed){name.value=saved.name;name.disabled=false;updateAnswer();status.textContent='Name saved with this assessment on this device.';}
