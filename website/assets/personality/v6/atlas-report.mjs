...
function figureCard(person,featured=false){
  ...
  const img=el('img',null,{src:`/assets/personality/v6/${person.image}`,alt:`Portrait of ${person.name}`,loading:featured?'eager':'lazy',width:'420',height:'420'});
...
function symbolsChapter(story){
  const node=section('story-symbols','05 / Story motifs','The symbolic coda.','Birthday signs and tarot give the report a visual rhythm. They remain separate from measured answers.');
  if(!story?.value){node.append(p('Open the story studio to add a birthday symbol or draw three cards.','atlas-empty'));return node;}
  const {value,sources}=story;
  const symbols=el('div',null,{class:'atlas-symbol-grid'});
  if(value.birthday){
    const result=birthdaySymbols(value.birthday);
    symbols.append(symbolCard('Western sun sign',result.western.sign??result.western.candidates.join(' / '),result.western.status==='boundary_sensitive_unresolved'?'Boundary date; sign unresolved without birth time and place.':'Approximate tropical date convention.'));
    symbols.append(symbolCard('Chinese zodiac year',result.chinese.animal??'Unavailable',result.chinese.convention));
  }
  ...
  if(value.draw){
    ...
    for(const card of value.draw.cards){const definition=sources.deck.cards.find(item=>item.id===card.cardId),position=sources.deck.spread.find(item=>item.id===card.positionId);
      const figure=el('figure');figure.append(el('img',null,{src:`/assets/personality/v6/tarot/${definition.asset.pngPath.split('/').at(-1)}`,alt:definition.asset.alt,width:'320',height:'500'}));
  ...
  if(!symbols.children.length&&!value.draw)node.append(p('Add a birthday or open the story studio to see this chapter.','atlas-empty'));
...
export function renderAtlas(report,story=null){
  ...
  const hero=el('div',null,{class:'atlas-hero-art'});hero.append(el('img',null,{src:'/assets/personality/v6/media/atlas-collage.png',alt:'Abstract overlapping colors and three illustrated cards',width:'1536',height:'1024'}));overview.append(hero);
