...
5. **Coda / The story they chose — tone: playful and clearly symbolic.** If an optional type preference is complete, describe the four axes as an unvalidated editorial pilot, not an official MBTI result. If a public-life parallel is supplied, use only the supplied biography summary and credited image. Say plainly that this person's type was not verified and the comparison is a metaphor, not evidence that the participant resembles them. If tarot or zodiac symbols are supplied, display only those supplied. Follow story.symbolInterpretation: when it says to show artwork only, do not add a reading; otherwise use the original prompts as open-ended reflection. Never treat symbols as personality evidence, prediction, cause, or a claim about a culture. Omit absent motifs without filler.
...
- Context fields were entered by the participant and included when they downloaded this kit. They are self-described facts or preferences only. Do not infer psychological traits, ethnicity, culture, politics, ability, or potential from birthplace, ancestry, race, religion, gender, age, food traditions, or birthday. Do not invent any missing identity detail.
...
- Write the main account in third person, using the supplied report name where useful. If a BG02 answer specifies pronouns, follow that answer; “report name only” means avoid third-person pronouns. If no pronouns were supplied, use singular “they.” BG32 and BG34 answers may guide tone and illustration density, without overriding this script or the evidence rules. Do not claim to be a psychologist, to have interviewed the participant, or to have independently verified their context. Mention incomplete coverage only when it limits an important conclusion.
...
const publicUrl=relative=>`https://engmanager.xyz/assets/personality/v6/${relative}`;
...
export function createReportKit(input,options={}){
  ...
  if(storyInput){
    ...
    const symbolChoices=[...(value.birthday?['o01','o02']:[]),...(value.draw?['o03']:[])];
    const answers={...value.background};
    if(symbolChoices.length)answers.bg35={status:'answered',selected:symbolChoices};else delete answers.bg35;
    story=prepareStoryPacket({bank:sources.bank,countries:sources.countries,deck:sources.deck,answers,
      approvedIds:Object.keys(answers).filter(id=>id!=='bg35'&&answers[id]?.status==='answered'),displayName:options.name??'',includeName:Boolean(options.name?.trim()),birthday:value.birthday||null,
      approvedSymbols:[...(value.birthday?['western','chinese']:[]),...(value.draw?['tarot']:[])],draw:value.draw});
