...
export function presentationForURL(href) {
  ...
  return ['s', 'r', 'e'].some(key => url.searchParams.has(key) || fragment.has(key)) ? 'v1' : url.searchParams.get('presentation')==='classic' ? 'v4' : 'v5';
}
...
export async function bootPresentation(href, {document: doc, load} = {}) {
  ...
  if (stylesheet) stylesheet.media = version !== 'v1' ? 'all' : 'not all';
  ...
  else if(version==='v4') await import('../v4/app.mjs');
