...
import {mountReportStudio} from './studio.mjs';
...
function report(){
  disposeReportKit=mountReportStudio(main,{state:clone(state),model:reportModel(),enhancement:currentEnhancement(),recordId:record?.id??null,canSave:Boolean(record?.id&&db&&!saveFailure&&!readOnly),flush,pendingWrites:pendingKit,onPending:promise=>{pendingKit=promise;},onShare:()=>navigate('share'),onBackup:backup});
}
