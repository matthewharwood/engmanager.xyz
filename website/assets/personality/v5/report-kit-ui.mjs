...
export function mountReportKit(root,{state,enhancement=null,recordId=null,canSave=false,flush=async()=>{},pendingWrites=Promise.resolve(),onPending=()=>{},story=()=>null,storyReady=Promise.resolve()}) {
  ...
  panel.append(el('p','Create your reading copy',{class:'eyebrow'}),el('h2','One story, one printable PDF.',{id:'report-kit-title'}),el('p','The kit combines your measured answers with only the story details and symbols you approved. Its five-movement script gives each PDF the same shape while leaving room for your own experience. The result is AI-written; you review it before sharing.'));
  const steps=el('ol',null,{class:'kit-steps'});
  for(const [title,body] of [
    ['Choose your story details','Use the optional studio above. Every background answer and symbol starts excluded from the AI kit until you explicitly include it.'],
  ...
  const make=()=>createReportKit(state,{name:name.value,context:context.value,enhancement,includeReflection:include.checked,story:story()});
