import {RELEASE} from '/assets/personality/v3/release.mjs';
import {AI_CACHE} from '/assets/personality/v1/ai-cache-config.mjs';
import {releaseSource} from '/assets/personality/v3/release-format.mjs';

const ASSET_ROOT = '/assets/personality/v3/';
...
const READY_PATH = '/personality/.offline-ready-v3';
const LEGACY_READY_PATH = '/personality/.offline-ready-v1';
const PREVIOUS_READY_PATH = '/personality/.offline-ready-v2';
...
const cacheNamePromise = digestPromise.then(digest => `personality-v3-${digest}`);
...
async function installRelease(progress) {
  ...
  try {
    ...
    await target.put(urlFor(READY_PATH), new Response(JSON.stringify({v: 3, releaseDigest: digest, assets: publicPaths.size, routes: ROUTES.length}), {
  ...
  event.waitUntil((async () => {
    ...
    try {
      ...
      if (source.origin !== origin || !INSTALL_SOURCES.has(source.pathname) ||
          Object.keys(event.data).length !== 2 || ![digest, RELEASE.legacy.releaseDigest, RELEASE.previous.releaseDigest].includes(event.data.releaseDigest)) {
        throw new Error('Reload this assessment before installing its offline release.');
      }
      // Frozen v1 and v2 clients can install this compatible superset. Echo
...
async function legacyCache() {
  const name = `personality-v1-${RELEASE.legacy.releaseDigest}`;
  if (!(await caches.keys()).includes(name)) return null;
  const cache = await caches.open(name);
  return await cache.match(urlFor(LEGACY_READY_PATH)) ? cache : null;
}

async function previousCache() {
  const name = `personality-v2-${RELEASE.previous.releaseDigest}`;
  if (!(await caches.keys()).includes(name)) return null;
  const cache = await caches.open(name);
  return await cache.match(urlFor(PREVIOUS_READY_PATH)) ? cache : null;
}

async function cachedPublic(path) {
  const current = await (await installedCache())?.match(urlFor(path));
  if (current) return current;
  if (ROUTES.includes(path) || path.startsWith(RELEASE.previous.root) || path.startsWith(RELEASE.legacy.root)) {
    const previous = await (await previousCache())?.match(urlFor(path));
    if (previous) return previous;
  }
  if (ROUTES.includes(path) || path.startsWith(RELEASE.legacy.root)) {
    return await (await legacyCache())?.match(urlFor(path));
  }
  return undefined;
}
