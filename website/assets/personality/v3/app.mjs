...
let reflectionContext=createContext(),enhancement=null,disposeReflection=null,disposeReportKit=null;
...
function report(){
  const header=heading('04 / YOUR REPORT','The person behind the numbers.','Your results are the evidence. A thoughtful narrative helps you explain what they might mean—and what someone could easily miss about you.');
  header.classList.add('report-hero');
  main.append(header);
  const model=reportModel();
  disposeReportKit=mountReportKit(main,{state:clone(state),enhancement:currentEnhancement(),recordId:record?.id??null,canSave:Boolean(db&&!saveFailure&&!readOnly),flush,pendingWrites:pendingKit,onPending:promise=>{pendingKit=promise;}});
  ...
  if(!readOnly){
    ...
    for(const experiment of EXPERIMENTS){const input=el('input',null,{type:'checkbox',checked:state.blocks.includes(experiment.id)});input.addEventListener('change',()=>{state.blocks=EXPERIMENTS.filter(e=>e.id===experiment.id?input.checked:state.blocks.includes(e.id)).map(e=>e.id).sort();persist();renderReport(content,reportModel());});panel.append(el('label',null,{class:'check-line'},[input,document.createTextNode(experiment.title)]));}
  ...
  const files=el('details',null,{class:'report-optional-files'});files.append(el('summary','Backups and other exports'),p('The report kit is a writing prompt with your evidence. JSON backups are for restoring this assessment in the app. The scores PDF contains your numeric report and any kept local reflection. The portrait PDF is produced by your chosen LLM from the kit.'));
...
function render(){
  disposeReflection?.();disposeReflection=null;disposeReportKit?.();disposeReportKit=null;
