...
export function mountReportKit(root,{state,enhancement=null,recordId=null,canSave=false,flush=async()=>{},pendingWrites=Promise.resolve(),onPending=()=>{}}) {
  ...
  panel.append(el('p','01 / FROM RESULTS TO A PERSONAL NARRATIVE',{class:'eyebrow'}),el('h2','A personal portrait. A PDF to keep.',{id:'report-kit-title'}),el('p','Give your chosen AI the complete evidence and a focused writing brief. Get a concise portrait of you, an editable first-person introduction, and a carefully typeset PDF with your scores at the back.'));
  const steps=el('ol',null,{class:'kit-steps'});
  for(const [title,body] of [
    ['Download your kit','One Markdown file includes the portrait brief, every selected answer, all available scores and facets, and the source notes. It is the input for your AI, not the finished PDF.'],
    ['Attach it to a fresh chat','Choose an AI chat that can create downloadable files. Attach the Markdown file and ask: Follow this brief and create my portrait PDF. Attaching the file avoids long pasted text being clipped.'],
    ['Read, refine, and keep your PDF','The brief asks for a direct personal portrait and readable score tables, with careful typography and no website. Review the interpretation and ask for changes before sharing. If the chat cannot create files, it will give you report text to export as PDF elsewhere.']
  ...
  fields.append(field('Name in the generated report',name,'Leave blank for “A portrait of you.”'),field('What would you like someone to understand?',context,'Only write details you want included in the downloaded file. These notes do not change your scores.'));
  ...
  downloadButton.addEventListener('click',()=>action(kit=>{download(kit);status.textContent='Report kit ready in your downloads. Attach the file to a fresh chat that can create files and ask: Follow this brief and create my portrait PDF.';}));
  ...
  copyButton.addEventListener('click',()=>action(async kit=>{
    try { await navigator.clipboard.writeText(kit.text);status.textContent='Complete prompt and evidence copied. If the chat clips a long paste, attach the downloaded kit instead. Ask it to follow the brief and create your portrait PDF.'; }
