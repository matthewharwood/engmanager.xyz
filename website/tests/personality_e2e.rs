...
try{
  ...
  doc().querySelector('#approve-bg04').click();
  ...
  assert(doc().querySelector('#bg04-self-description').value==='A fictional self-description'&&doc().querySelector('#approve-bg04').checked,'self-description and field approval restore');
  doc().querySelector('#bg04-o10').closest('fieldset').querySelector('button:nth-child(2)').click();
  assert(!doc().querySelector('#approve-bg04').checked&&doc().querySelector('#bg04-self-description').closest('.atlas-self-description').hidden,'clearing an answer also clears its export approval');
  doc().querySelector('#bg01-o01').click();
  doc().querySelector('#bg01-o01').closest('fieldset').querySelector('button:first-child').click();
  assert(doc().querySelector('#bg01-o01').closest('fieldset').textContent.includes('Skipped for now. You can answer later.'),'optional answers can be explicitly skipped');
  doc().querySelector('#bg01-o01').closest('fieldset').querySelector('button:nth-child(2)').click();
  ...
  click('Explore your own profile');await until(()=>doc().querySelector('#module-big5')&&!doc().querySelector('.summary-scores'),'leave shared summary');
  assert(!doc().querySelector('#module-interests').disabled,'summary escape opens editable own workspace');
  assert((await store.loadActive()).state.responses[0]===4,'summary escape preserves original answers');
  const wording=doc().querySelector('#wording');wording.value='she';wording.dispatchEvent(new frame.contentWindow.Event('change',{bubbles:true}));
  ...
  const {createReportKit}=await import('/assets/personality/v5/report-kit.mjs');
  ...
  click('Explore your own profile');await until(()=>doc().querySelector('#module-big5'),'own workspace after public summary');
  ...
  doc().querySelector('#bg04-o01').click();doc().querySelector('#approve-bg04').click();
  ...
  doc().querySelector('#symbol-western').click();doc().querySelector('#symbol-approve-western').click();
  doc().querySelector('#symbol-tarot').click();doc().querySelector('#symbol-approve-tarot').click();
  click('Draw three cards');
  ...
  const storyKit=await kitDownload('approved story kit download');
  assert(storyKit.text.includes('your-story-packet-v2')&&storyKit.text.includes('"id": "bg04"'),'approved background answer enters kit');
  assert(storyKit.text.includes('"sign": "Virgo"')&&storyKit.text.includes('"cards": ['),'approved symbols and saved draw enter kit');
  ...
  assert(doc().querySelector('#approve-bg04').checked&&doc().querySelectorAll('.atlas-mini-spread figure').length===3,'approval and draw restore with the draft');
