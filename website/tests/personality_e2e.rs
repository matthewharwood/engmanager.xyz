...
try{
  ...
  assert(doc().querySelector('#report-kit')&&doc().querySelector('.report-hero').textContent.includes('How to work with this person.'),'report leads with company-facing workplace workflow');
  ...
  const {createReportKit}=await import('/assets/personality/v4/report-kit.mjs');
  ...
  assert(downloadedKit.text.includes('Return one real, downloadable PDF')&&!downloadedKit.text.includes('standalone HTML document'),'download requests a real workplace PDF rather than an HTML artifact');
  assert(downloadedKit.text.includes('occupational psychologist briefing a company')&&downloadedKit.text.includes('Write the MAIN REPORT in third person'),'download uses third-person workplace psychology briefing');
