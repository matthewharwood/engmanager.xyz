...
As part of [Auteurs](/articles/auteurs) — our guild of engineers, designers, and product folks who turn conversations into shipped product — we've been building our own. Not a WordPress skin: a real one. Server-rendered Rust, a JSX-like templating macro, Stripe checkout that never leaves the page, every asset compiled into a single binary. It's the engine behind this very site and the [merch store](/articles/vibe-coding-a-shop). Point an MCP at it and one person can mass-create blocks; we've been writing *skills* so the AI builds them faster and breaks them less. One person can comfortably run five or six sites this way.

That's a good tool. But a tool is only worth what you aim it at — and I want to aim ours at [Project FootTraffic](/articles/project-foottraffic).

Looking for the first place to point it, I found [Rune and Board](https://runeandboard.com): a family game shop out in Hillsboro, a play on "Room and Board." The site looks like it was assembled between shifts, but underneath it's already doing a *ton* — real inventory, real commerce. Our CMS could make this place badass. And shops like this have the money; a busy one runs a few thousand dollars a day through the register. What they don't have is a dev. We have a CMS, and a way to run a lot of them at once.
...
<aside class="article-callout">
  <strong>The upgrade:</strong> turn a CMS that ships one site at a time into the spine of a three-sided marketplace — where a portfolio of small businesses becomes a standing inventory of demand that clears like an order book.
</aside>
...
The model I keep drawing looks like [rentahuman.ai](https://rentahuman.ai) — but pointed inward, at our own real estate. Three sides.
...
<div class="wf-job"><span class="wf-job-status is-claimed">CLAIMED</span><span class="wf-job-main"><span class="wf-job-title">Menu redesign + photo intake</span><span class="wf-job-meta"><span class="wf-chip wf-chip-biz">Cafe Oso</span><span class="wf-chip wf-chip-tag">content</span></span></span><span class="wf-claimed">@operator</span></div>
<div class="wf-job"><span class="wf-job-status is-house">HOUSE</span><span class="wf-job-main"><span class="wf-job-title">New brand system + POS extension</span><span class="wf-job-meta"><span class="wf-chip wf-chip-biz">Rune &amp; Board</span><span class="wf-chip wf-chip-tag">brand</span><span class="wf-chip wf-chip-tag">block</span></span></span><span class="wf-claimed">The House</span></div>
</div>
</div>
...
flowchart TD
  ...
  HOUSE --> CAP["Capture reusable block"]
  AUTO --> DONE["Shipped"]
  OPEN --> DONE
  CAP --> DONE
</div>
...
flowchart LR
  ...
  D --> E["More jobs, more captured blocks"]
...
<div class="workflow">
  ...
  <div class="workflow-arrow" aria-hidden="true">→</div>
  <div class="workflow-stage workflow-stage-single">Block capture</div>
</div>
...
- **The router** — the classifier that sends a job to a CMS tool, the network, or the house. The IP.
- **Self-serve tiles** — booking, loyalty, post-visit, discounts and gift cards, exposed as one-tap CMS blocks. The assembly line we've been sketching all along.
- **Block capture** — every house job ends by promoting its reusable part back into the CMS.
- **Billing** — subscriptions on top of the Stripe wiring the CMS already has. We don't beat Shopify's margin, and we're not trying to.
...
<aside class="article-callout">
  <strong>This is where you come in.</strong> Tell me which brick is wrong, which one's missing, and which one you want to own. Same as always — we build it brick by brick.
</aside>
