#!/usr/bin/env node
// Published v1, optional ai/v1, and v2 are immutable. This command verifies them,
// then generates the independent v3 presentation/offline manifest only.
...
export async function generatePresentationRelease({allowIncomplete = false} = {}) {
  ...
  const {RELEASE: previous} = await import('../website/assets/personality/v2/release.mjs');
  const root = path.join(assetsRoot, 'v3');
  ...
  const previousManifest = await readFile(path.join(assetsRoot, 'v2/release.mjs'));
  const assets = Object.fromEntries([
    ...Object.entries(previous.assets),
    ['/assets/personality/v2/release.mjs', digest(previousManifest)],
    ...Object.entries(current).map(([name, hash]) => [`/assets/personality/v3/${name}`, hash]),
  ].sort(([a], [b]) => a < b ? -1 : a > b ? 1 : 0));
  const release = {
    v: 3,
    presentation: 'portrait-pdf-v3',
    legacy: {root: '/assets/personality/v1/', manifestSha256: digest(baseManifest), releaseDigest: digest(JSON.stringify(base))},
    previous: {root: '/assets/personality/v2/', manifestSha256: digest(previousManifest), releaseDigest: digest(JSON.stringify(previous))},
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (args.includes('--verify-published')) {
    await verifyPublishedReleases();
    console.log('Published v1, ai/v1, and v2 bytes verified unchanged.');
  } else {
    const release = await generatePresentationRelease({allowIncomplete: args.includes('--allow-incomplete')});
    console.log(`Personality v3 presentation: ${Object.keys(release.assets).length} public assets; ${release.ready ? 'complete' : 'incomplete development manifest'}. Published releases unchanged.`);
  }
}
