...
import {RELEASE as previous} from '../website/assets/personality/v2/release.mjs';
import {RELEASE as current} from '../website/assets/personality/v3/release.mjs';
...
import {presentationForURL, bootPresentation} from '../website/assets/personality/v3/bootstrap.mjs';
import {releaseSource} from '../website/assets/personality/v3/release-format.mjs';
...
test('published v1, optional AI, and v2 bytes match their immutable release locks', async () => {
  ...
  assert.deepEqual(Object.keys(lock.roots), ['v1', 'ai/v1', 'v2', 'v3']);
});
...
test('v3 inventory includes exact legacy assets and no model runtime or respondent lookup paths', async () => {
  assert.equal(current.v, 3);
  assert.equal(current.presentation, 'portrait-pdf-v3');
  assert.equal(current.previous.releaseDigest, hash(JSON.stringify(previous)));
  ...
  const priorManifest = await readFile(new URL('../website/assets/personality/v2/release.mjs', import.meta.url));
  assert.equal(current.previous.manifestSha256, hash(priorManifest));
  assert.equal(current.assets['/assets/personality/v2/release.mjs'], hash(priorManifest));
  ...
  for (const [path, digest] of Object.entries(current.assets)) {
    assert.match(path, /^\/assets\/personality\/v[123]\/[A-Za-z0-9._/-]+$/);
    assert(!path.includes('..') && !path.includes('/ai/'));
    const bytes = await readFile(new URL(`../website${path}`, import.meta.url));
    assert.equal(hash(bytes), digest, `Stale presentation asset ${path}; generate v3 manifest after final edits.`);
  }
});
...
test('the entry point selects only hardcoded presentations and does not accept malformed packets', async () => {
  ...
  for (const suffix of ['', '#section', '?utm_source=example']) {
    assert.equal(presentationForURL('https://engmanager.xyz/personality/report' + suffix), 'v3');
  }
  ...
  assert.equal(body.dataset.personalityPresentation, 'v3');
  ...
  assert.deepEqual(versions, ['v3', 'v1']);
});
...
test('deployment verifier parses only the known data wrapper and rejects unsafe release paths', () => {
  ...
  for (const unsafe of ['https://other.test/app.mjs', '/assets/personality/v3/../v1/app.mjs', '/assets/personality/v3/app.mjs?answer=5']) {
